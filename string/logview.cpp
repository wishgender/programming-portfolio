...
char largePath[62] =  "/users/kent/faculty/jmaletic/public_html/data/log_4_large.txt";

int main(int argc, char *argv[]) {
    // Options
    std::vector<String> option(8);
    option[0] = "all";
    option[1] = "bytes";
    option[2] = "host";
    option[3] = "ip";
    ...
    if (argv[1] == option[4] || argv[1] == option[5] || argv[1] == option[6] || argv[1] == option[7]) {
        argv[2] = largePath;
        argc = 3;
    }
    ...
    if (argv[1] == option[0] || argv[1] == option[4]) {                // Handle the specified option
        printAllEntries(log_entries);
      //  output_all(std::cout, log_entries);
    }
    else if (argv[1] == option[1] || argv[1] == option[5]) {
        std::cout << "Total number of bytes sent: " << byte_count(log_entries) << std::endl;
    }
    else if (argv[1] == option[2] || argv[1] == option[6]) {
        by_host(std::cout, log_entries);
    }
    else if (argv[1] == option[3] || argv[1] == option [7]) {
        reportIP(log_entries);
    }
